...
def draw_bezier_curves(graph, ax, final_adj_list, sorted_tree_edges, parent_edge, height, side):

    position = nx.get_node_attributes(graph, 'pos')
    nodes = nx.draw_networkx_nodes(graph, position, node_size=200, node_color='lightgray', ax=ax)
    labels = nx.draw_networkx_labels(graph, position, labels={n: n for n in graph}, ax=ax)
