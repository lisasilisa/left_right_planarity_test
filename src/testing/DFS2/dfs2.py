...
def dfs2(v, sorted_adj_list, parent_edge, stack_bottom, stack, low_pt_edge, low_pt, low_pt_2, height, ref, side):
    ...
    for e_i in sorted_adj_list[v]:
        ...
        if e_i == parent_edge[e_i[1]]:
            planar = dfs2(e_i[1], sorted_adj_list, parent_edge, stack_bottom, stack, low_pt_edge, low_pt, low_pt_2, height, ref, side)
            if not planar:
                return False
        else:
            low_pt_edge[e_i] = e_i
            stack.push([[(math.nan, math.nan), (math.nan, math.nan)], [e_i, e_i]])  # , math.nan, , math.nan
